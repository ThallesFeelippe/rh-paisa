    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade text-on-background">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 ml-12">Controle completo de colaboradores, planos de carreira e documentos.</p>
        </div>
        {!isFormOpen && (
          <button 
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4">
             <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Colaboradores</p>
                <p className="text-3xl font-headline font-black text-emerald-950 italic">{employees.length}</p>
             </div>
             <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-1 border-emerald-200 bg-emerald-50/20">
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Verificação (Filtro)</p>
                <p className="text-[10px] font-bold text-slate-500">Mostrando isApprentice: FALSE</p>
                <p className="text-xs font-black text-emerald-950 uppercase mt-2">Clique em Jovem Aprendiz no menu ao lado para ver se eles estão lá por engano.</p>
             </div>
          </div>
        </div>
      )}
